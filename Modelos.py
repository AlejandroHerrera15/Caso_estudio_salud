...
rd_model.compile(optimizer="adam", loss='categorical_crossentropy', metrics=["accuracy", "recall", "precision"])
#Entrenamiento
rd_model.fit(x_train, y_train1, epochs=20, validation_data=(x_test, y_test1))

#Evaluar el modelo
test_loss, test_acc, test_recall, test_precision = rd_model.evaluate(x_test, y_test1, verbose=2)
print("Test recall:", test_recall)
rd_model.predict(x_test)
#Matriz de confusión
#evaluar algun porcentaje como filtro____pred_test=(rd_model.predict(x_test) > 0.50 ).astype("int")
#pred_test=(rd_model.predict(x_test) > 0.80 ).astype("int")
...
cm=metrics.confusion_matrix(y_test2, pred_test1)
disp=metrics.ConfusionMatrixDisplay(cm,display_labels=['no_tumor', 'benigno_tumor','maligno_tumor','pituitary_tumor' ])
disp.plot()
print(metrics.classification_report(y_test2, pred_test1))
...
#Optimizador y funcion de perdida
rd_model2.compile(optimizer="adam", loss='categorical_crossentropy', metrics=["accuracy", "recall", "precision"])
#Entrenamiento
rd_model2.fit(x_train, y_train1, epochs=10, validation_data=(x_test, y_test1))

#Evaluar el modelo
test_loss, test_acc, test_recall, test_precision = rd_model2.evaluate(x_test, y_test1, verbose=2)
print("Test recall:", test_recall)
...
#pred_test2=(rd_model2.predict(x_test)> 0.10 ).astype("int")
...
cm=metrics.confusion_matrix(y_test2, pred_test2)
disp=metrics.ConfusionMatrixDisplay(cm,display_labels=['no_tumor', 'benigno_tumor','maligno_tumor','pituitary_tumor'])
disp.plot()
print(metrics.classification_report(y_test2, pred_test2))
