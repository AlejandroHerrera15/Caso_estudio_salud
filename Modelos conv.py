...
model_cov.compile(loss="categorical_crossentropy", optimizer="adam", metrics=["accuracy", "recall", "precision"])
#Entrenamiento
model_cov.fit(x_train, y_train1, batch_size=100, epochs=20, validation_data=(x_test, y_test1))

model_cov.summary()


#Evaluar el modelo
test_loss, test_acc, test_recall, test_precision = model_cov.evaluate(x_test, y_test1, verbose=2)
print("Test recall:", test_recall)
model_cov.predict(x_test)
#Matriz de confusión
#evaluar algun porcentaje como filtro____pred_test=(rd_model.predict(x_test) > 0.50 ).astype("int")
#pred_test=(model_cov.predict(x_test) > 0.80 ).astype("int")
#pred_test.shape #El modelo tiene una probabilidad mayor a 80% en todas las clases


pred_test=model_cov.predict(x_test)
#pred_test=np.argmax(pred_test, axis=1)
pred_test=np.argmax(pred_test, axis=1)
y_test1=np.argmax(y_test1, axis=1)

cm=metrics.confusion_matrix(y_test1, pred_test)
